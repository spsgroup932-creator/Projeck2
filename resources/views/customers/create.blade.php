                    ...
                    <form action="{{ route('customers.store') }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="row g-4 border-bottom border-white border-opacity-10 pb-5 mb-5">
                            <div class="col-md-6">
                                ...
                                <input type="text" name="nik" class="form-control glass-panel @error('nik') is-invalid @enderror" value="{{ old('nik') }}" placeholder="Contoh: 3312345678901234">
                                @error('nik') <div class="invalid-feedback">{{ $message }}</div> @enderror
                            </div>

                            <div class="col-md-6">
                                <label class="form-label text-white fw-medium">Nama Lengkap *</label>
                                <input type="text" name="name" class="form-control glass-panel @error('name') is-invalid @enderror" value="{{ old('name') }}">
                                @error('name') <div class="invalid-feedback">{{ $message }}</div> @enderror
                            </div>

                            <div class="col-md-6">
                                <label class="form-label text-white fw-medium">Nomor WhatsApp / HP</label>
                                <input type="text" name="phone" class="form-control glass-panel @error('phone') is-invalid @enderror" value="{{ old('phone') }}">
                                @error('phone') <div class="invalid-feedback">{{ $message }}</div> @enderror
                            </div>

                            <div class="col-md-6">
                                <label class="form-label text-white fw-medium">Alamat Domisili</label>
